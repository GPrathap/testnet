...
class Neotx():
    ...
    def get_neoxt_conv2d_transpose_layer(self, current_layers, depth, filters, apply_batch_normalization, is_train):
        next_layers = []
        for filter_size, current_layer in zip(filters, current_layers):
            network = tf.layers.conv2d_transpose(current_layer, depth, [filter_size, filter_size], strides=(2, 2)
    ...
    def generator(self, inputs, is_train=True, reuse=False):
        with tf.variable_scope("generator", reuse=reuse):
            ...
            h1_layers = self.get_neoxt_generator_reshape_layer(inputs, self.init_depth_of_generator
                                                               , self.filters, True, is_train)

            depth_of_h1 = int(self.init_depth_of_generator/2)
            h1_layers = self.get_neoxt_conv2d_transpose_layer(h1_layers, depth_of_h1, self.filters, True, is_train)

            depth_of_h2 = int(depth_of_h1/2)
            h2_layers = self.get_neoxt_conv2d_transpose_layer(h1_layers, depth_of_h2, self.filters, True, is_train)

            depth_of_h3 = int(depth_of_h2/2)
            h3_layers = self.get_neoxt_conv2d_transpose_layer(h2_layers, depth_of_h3, self.filters, True, is_train)

            depth_of_h4 = int(depth_of_h3/2)
            h4_layers = self.get_neoxt_conv2d_transpose_layer(h3_layers, depth_of_h4, self.filters, True, is_train)

            depth_of_h5 = int(depth_of_h4/2)
            h5_layers = self.get_neoxt_conv2d_transpose_layer(h4_layers, depth_of_h5, self.filters, True, is_train)

            depth_of_h6 = int(depth_of_h5/2)
            h6_layers = self.get_neoxt_conv2d_transpose_layer(h5_layers, depth_of_h6, self.filters, True, is_train)
    ...
    def discriminator(self, inputs, is_train=True, reuse=False):
        ...
        with tf.variable_scope("discriminator", reuse=reuse):
            net_h0 = self.get_neoxt_conv2d_first_layer(inputs, self.init_depth_of_discriminator, self.filters, False, is_train)

            depth_of_h1 = self.init_depth_of_discriminator*2
            net_h1 = self.get_neoxt_conv2d_layer(net_h0, depth_of_h1, self.filters, True, is_train)

            depth_of_h2 = depth_of_h1*2
            net_h2 = self.get_neoxt_conv2d_layer(net_h1, depth_of_h2, self.filters, True, is_train)

            depth_of_h3 = depth_of_h2*2
            net_h3 = self.get_neoxt_conv2d_layer(net_h2, depth_of_h3, self.filters, True, is_train)
            feature_set.append(tf.concat(self.get_neoxt_features(net_h3), axis=1))

            depth_of_h4 = depth_of_h3 * 2
            net_h4 = self.get_neoxt_conv2d_layer(net_h3, depth_of_h4, self.filters, True, is_train)
            feature_set.append(tf.concat(self.get_neoxt_features(net_h4), axis=1))

            depth_of_h5 = depth_of_h4 * 2
            net_h5 = self.get_neoxt_conv2d_layer(net_h4, depth_of_h5, self.filters, True, is_train)
