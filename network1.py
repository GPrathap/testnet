...
def generator_simplified_api(inputs, batch_size, is_train=True, reuse=False):
    ...
    with tf.variable_scope("generator", reuse=reuse):
        ...
        net_h0 = tf.contrib.layers.batch_norm(net_h0, activation_fn=tf.nn.relu, is_training=is_train,
                                               reuse=reuse, scope='g/h0/batch_norm')
        ...
        net_h1 = tf.contrib.layers.batch_norm(net_h1, activation_fn=tf.nn.relu, is_training=is_train,
                                              reuse=reuse, scope='g/h1/batch_norm')
        ...
        net_h2 = tf.contrib.layers.batch_norm(net_h2, activation_fn=tf.nn.relu, is_training=is_train,
                                              reuse=reuse, scope='g/h2/batch_norm')
        ...
        net_h3 = tf.contrib.layers.batch_norm(net_h3, activation_fn=tf.nn.relu, is_training=is_train,
                                              reuse=reuse, scope='g/h3/batch_norm')
        ...
        net_h4 = tf.contrib.layers.batch_norm(net_h4, activation_fn=tf.nn.relu, is_training=is_train,
                                              reuse=reuse, scope='g/h4/batch_norm')
        ...
        net_h5 = tf.contrib.layers.batch_norm(net_h5, activation_fn=tf.nn.relu, is_training=is_train,
                                              reuse=reuse, scope='g/h5/batch_norm')
...
def discriminator_simplified_api(inputs, is_train=True, reuse=False):
    ...
    with tf.variable_scope("discriminator", reuse=reuse):
        ...
        net_h1 = tf.contrib.layers.conv2d(net_h1, df_dim*2, [k, k], stride=2, padding='SAME',
                                          weights_initializer=w_init, reuse=reuse, trainable=is_train,
                                          activation_fn=None, scope='g/h1/conv2d')
        ...
        net_h2 = tf.contrib.layers.conv2d(net_h1, df_dim * 4, [k, k], stride=2, padding='SAME',
                                          weights_initializer=w_init, reuse=reuse, trainable=is_train,
                                          activation_fn=None, scope='g/h2/conv2d')
        ...
        net_h3 = tf.contrib.layers.conv2d(net_h2, df_dim * 8, [k, k], stride=2, padding='SAME',
                                          weights_initializer=w_init, reuse=reuse, trainable=is_train,
                                          activation_fn=None, scope='g/h3/conv2d')
        ...
        global_max1 = tf.contrib.layers.max_pool2d( net_h3, [4,4], stride=1, padding='SAME',
                                                    outputs_collections=None, scope='g/h3/max_pool2d')
        global_max1 = tf.contrib.layers.flatten(global_max1, scope='g/h3/flatten')

        net_h4 = tf.contrib.layers.conv2d(net_h3, df_dim * 16, [k, k], stride=2, padding='SAME',
                                          weights_initializer=w_init, reuse=reuse, trainable=is_train,
                                          activation_fn=None, scope='g/h4/conv2d')
        ...
        global_max2 = tf.contrib.layers.max_pool2d(net_h4, [2, 2], stride=1, padding='SAME',
                                                   outputs_collections=None, scope='g/h4/max_pool2d')
        global_max2 = tf.contrib.layers.flatten(global_max2, scope='g/h4/flatten')

        net_h5 = tf.contrib.layers.conv2d(net_h4, df_dim * 32, [k, k], stride=2, padding='SAME',
                                          weights_initializer=w_init, reuse=reuse, trainable=is_train,
                                          activation_fn=None, scope='g/h5/conv2d')
        net_h5 = tf.contrib.layers.batch_norm(net_h5, activation_fn=lambda x: tf.nn.leaky_relu(x, 0.2),
                                              is_training=is_train,
                                              reuse=reuse, scope='g/h5/batch_norm')
        global_max3 = tf.contrib.layers.flatten(net_h5, scope='g/h5/flatten')
