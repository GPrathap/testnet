...
def batch_normalization_layer(layer, gamma_init, is_training, is_trainable, reuse):
...
def generator_simplified_api(inputs, batch_size, is_train=True, reuse=False):
    ...
    with tf.variable_scope("generator", reuse=reuse):
        ...
        net_h0 = batch_normalization_layer(net_h0, gamma_init, is_train, is_train, reuse=reuse)

        net_h1 = tf.layers.conv2d_transpose(
            net_h0, gf_dim*16, [k, k], strides=(2,2), padding='SAME', activation=None)
        net_h1 = batch_normalization_layer(net_h1, gamma_init
        ...
        net_h2 = batch_normalization_layer(net_h2, gamma_init
                                           , is_train, is_train, reuse=reuse)

        net_h3 = tf.layers.conv2d_transpose(
            net_h2, gf_dim * 4, [k, k], strides=(2,2), padding='SAME', activation=None)
        net_h3 = batch_normalization_layer(net_h3, gamma_init, is_train, is_train, reuse=reuse)

        net_h4 = tf.layers.conv2d_transpose(
            net_h3, gf_dim * 2, [k, k], strides=(2,2), padding='SAME', activation=None)
        net_h4 = batch_normalization_layer(net_h4, gamma_init,is_train, is_train, reuse=reuse)

        net_h5 = tf.layers.conv2d_transpose(
            net_h4, gf_dim * 1, [k, k], strides=(2,2), padding='SAME', activation=None)
        net_h5 = batch_normalization_layer(net_h5, gamma_init, is_train, is_train, reuse=reuse)

        net_h6 = tf.layers.conv2d_transpose(net_h5, 3, [k, k], strides=(2,2), padding='SAME', activation=None)
...
def discriminator_simplified_api(inputs, is_train=True, reuse=False):
    ...
    with tf.variable_scope("discriminator", reuse=reuse):
        ...
        net_h1 = tf.layers.conv2d(net_h1, df_dim*2, [k, k], strides=(2,2), padding='SAME',
                                          activation=None)
        net_h1 = batch_normalization_layer(net_h1, gamma_init, is_train, is_train, reuse=reuse)

        net_h2 = tf.layers.conv2d(net_h1, df_dim * 4, [k, k], strides=(2,2), padding='SAME',
                                          activation=None)
        net_h2 = batch_normalization_layer(net_h2, gamma_init, is_train, is_train, reuse=reuse)
        ...
        global_max2 = tf.layers.flatten(global_max2)
