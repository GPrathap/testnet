...
def get_neoxt_conv2d_transpose_layer(current_layers, depth, filters, apply_batch_normalization, is_train):
    next_layers = []
    for filter_size, current_layer in zip(filters, current_layers):
        ...
        if apply_batch_normalization:
            gamma_init = tf.random_normal_initializer(1., 0.02)
            network = batch_normalization_layer(network, gamma_init, is_train)
