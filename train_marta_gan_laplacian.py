...
def main(_):
    ...
    z_dim = 100
    z_conditional_dim = 10

    # with tf.device("/gpu:0"): # <-- if you have a GPU machine
    z = tf.placeholder(tf.float32, [FLAGS.batch_size, z_dim + z_conditional_dim], name='z_noise')
    ...
    net_d, d_logits, feature_fake = conditional_discriminator_simplified_api(net_g.outputs
                                                                             , net_g.outputs, is_train=True, reuse=False)
    # real images --> discriminator
    net_d2, d2_logits, feature_real = conditional_discriminator_simplified_api(real_images, real_images, is_train=True, reuse=True)
    # sample_z --> generator for evaluation, set is_train to False
    # so that BatchNormLayer behave differently
    net_g2, g2_logits = conditional_generator_simplified_api(z,z, FLAGS.batch_size, is_train=False, reuse=True)

    #
    net_d3, d3_logits, _ = conditional_discriminator_simplified_api(real_images, real_images, is_train=False, reuse=True)
